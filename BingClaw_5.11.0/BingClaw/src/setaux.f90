...
subroutine setaux(mbc,mx,my,xlow,ylow,dx,dy,maux,aux)
	...
	! start of vp2 addtion
    real(kind=8) :: xc,yc,dx_meters,dy_meters
	! end of vp2 addtion
	...
	! start of vp2 addition

    aux(4:5,:,:)=0.d0

    do ii=1,mx

       xm = xlower + (ii - 1.d0) * dx
       xc = xlower + (ii - 0.5d0) * dx
       xp = xlower + ii * dx

       do jj=1,my

          ym = ylower + (jj - 1.d0) * dy
          yc = ylower + (jj - 0.5d0) * dy
          yp = ylower + jj * dy
          ...
          if (aux(1,ii,jj)<0.d0) then
             aux(4,ii,jj) = grav*(1.d0-rho_a/rho_s) &
                *dcos(atan((aux(1,ii+1,jj)-aux(1,ii-1,jj))/2.d0/dx_meters))
             aux(5,ii,jj) = grav*(1.d0-rho_a/rho_s) &
                *dcos(atan((aux(1,ii,jj+1)-aux(1,ii,jj-1))/2.d0/dy_meters))
          else
             aux(4,ii,jj) = grav* &
                dcos(atan((aux(1,ii+1,jj)-aux(1,ii-1,jj))/2.d0/dx_meters))
             aux(5,ii,jj) = grav* &
